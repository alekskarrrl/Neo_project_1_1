...
* [Задание 2.2](#задание-22)
  * [Демо 2.2](#демо-22)
  * [Реализация по заданию 2.2](#реализация-по-заданию-22)
...
Ссылка на видеозапись демонстрации:  [Demo Video](https://drive.google.com/file/d/1I3HrDfoGkX7k_5kVbjH_aAB2rVHT0fEf/view?usp=sharing)

<p align="right">(<a href="#readme-top">back to top</a>)</p>


### Реализация по заданию 2.2

#### Рабочие директории

`/opt/notebooks/logs` - для сохранения логов,  
`/opt/notebooks/data_deltas/` - хранятся подпапки с дельта-таблицами,  
`/opt/notebooks/mirr_md_account_d` - для сохранения конечного результата - зеркало таблицы `md_account_d` в формате csv.    

#### Необходимые входные параметры для приложения

Директории описаны выше:  
`logs_delta_path = "/opt/notebooks/logs"`  
`home_deltas_path = "/opt/notebooks/data_deltas/"`

`table_name = "md_account_d"` - имя таблицы, в которую загружаем дельты,   
`p_keys = ["account_rk"]` - первичный ключ таблицы (по заданию),  
`p_keys = ["data_actual_date", "account_rk"]` - первичный ключ таблицы (такой ключ установлен для таблицы `md_account_d` на предыдущих этапах задания). В демонстрации рассмотрены оба варианта певричного ключа.  


#### Код для решения  
Для этого задания рабочей папкой можно считать `/opt/notebooks`.  

- project_2_2_alter_table.sql - SQL запрос для добавления столбцов в исходную таблицу в Postgres  
- project_2_2.ipynb - решение, оформленное в Jupyter Notebook  
- project_2_2_py.py - файл для исполнения через консоль. Код - копия кода из `project_2_2.ipynb` с добавлением аргументов из консоли.  

#### Для запуска в консоли выполнить:  

  ```shell
  ./bin/spark-submit \
  --conf "spark.driver.extraClassPath=/opt/hadoop/postgresql-42.6.0.jar" \
  --name "project_2_2" \
  /opt/notebooks/project_2_2_py.py \
  "md_account_d" \
  "/opt/notebooks/data_deltas/" \
  "/opt/notebooks/logs" \
  "account_rk"
  ```  

или, в случае добавления поля к первичному ключу:  
